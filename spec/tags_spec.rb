...
describe 'compiled component amazonmq-rabbitmq' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/tags/amazonmq-rabbitmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "SecurityGroup" do
      ...
      it "to have property Tags" do
          expect(resource["Properties"]["Tags"]).to eq([{"Key"=>"Environment", "Value"=>{"Ref"=>"EnvironmentName"}}, {"Key"=>"EnvironmentType", "Value"=>{"Ref"=>"EnvironmentType"}}, {"Key"=>{"Fn::Sub"=>"name1"}, "Value"=>{"Fn::Sub"=>"value2"}}])
      end
