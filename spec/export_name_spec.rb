...
describe 'compiled component amazonmq-rabbitmq' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/export_name.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/export_name/amazonmq-rabbitmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "SecurityGroup" do
      ...
      it "to have property GroupDescription" do
          expect(resource["Properties"]["GroupDescription"]).to eq({"Fn::Sub"=>"${EnvironmentName}-rabbit"})
      end
    ...
    context "PasswordSSMSecureParameter" do
      ...
      it "to have property Path" do
          expect(resource["Properties"]["Path"]).to eq({"Fn::Sub"=>"/rabbit/${EnvironmentName}/password"})
      end
    ...
    context "UsernameParameterSecretKey" do
      ...
      it "to have property Name" do
          expect(resource["Properties"]["Name"]).to eq({"Fn::Sub"=>"/rabbit/${EnvironmentName}/username"})
      end
    ...
    context "Broker" do
      ...
      it "to have property BrokerName" do
          expect(resource["Properties"]["BrokerName"]).to eq({"Fn::Sub"=>"${EnvironmentName}-rabbit"})
      end
      ...
      it "to have property Users" do
          expect(resource["Properties"]["Users"]).to eq([{"Username"=>"administrator", "Password"=>{"Fn::GetAtt"=>["PasswordSSMSecureParameter", "Password"]}}])
      end
