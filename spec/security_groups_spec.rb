...
describe 'compiled component amazonmq-rabbitmq' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/security_groups/amazonmq-rabbitmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "SecurityGroup" do
      ...
      it "to have property SecurityGroupIngress" do
          expect(resource["Properties"]["SecurityGroupIngress"]).to eq([{"FromPort"=>5671, "IpProtocol"=>"TCP", "ToPort"=>5671, "Description"=>{"Fn::Sub"=>"allow access to amqps protocol"}, "CidrIp"=>{"Fn::Sub"=>"10.0.0.0/16"}}, {"FromPort"=>443, "IpProtocol"=>"TCP", "ToPort"=>443, "Description"=>{"Fn::Sub"=>"allow access to UI"}, "CidrIp"=>{"Fn::Sub"=>"10.0.0.0/16"}}])
      end
