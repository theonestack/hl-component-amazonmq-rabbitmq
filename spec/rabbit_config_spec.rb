...
describe 'compiled component amazonmq-rabbitmq' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/rabbit_config.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/rabbit_config/amazonmq-rabbitmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "PasswordSSMSecureParameter" do
      ...
      it "to have property Description" do
          expect(resource["Properties"]["Description"]).to eq({"Fn::Sub"=>"${EnvironmentName} RabbitMQ User my_new_username Password"})
      end
      
      it "to have property Tags" do
          expect(resource["Properties"]["Tags"]).to eq([{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-my_new_username-rabbitmq-password"}}, {"Key"=>"Environment", "Value"=>{"Fn::Sub"=>"${EnvironmentName}"}}, {"Key"=>"EnvironmentType", "Value"=>{"Fn::Sub"=>"${EnvironmentType}"}}])
      end
    ...
    context "UsernameParameterSecretKey" do
      ...
      it "to have property Value" do
          expect(resource["Properties"]["Value"]).to eq("my_new_username")
      end
    ...
    context "Broker" do
      ...
      it "to have property Logs" do
          expect(resource["Properties"]["Logs"]).to eq({"General"=>false})
      end
      
      it "to have property MaintenanceWindowStartTime" do
          expect(resource["Properties"]["MaintenanceWindowStartTime"]).to eq({"DayOfWeek"=>"Monday", "TimeOfDay"=>"22:45", "TimeZone"=>"America/Los_Angeles"})
      end
      
      it "to have property PubliclyAccessible" do
          expect(resource["Properties"]["PubliclyAccessible"]).to eq(true)
      end
      ...
      it "to have property StorageType" do
          expect(resource["Properties"]["StorageType"]).to eq("gp3")
      end
      ...
      it "to have property Users" do
          expect(resource["Properties"]["Users"]).to eq([{"Username"=>"my_new_username", "Password"=>{"Fn::GetAtt"=>["PasswordSSMSecureParameter", "Password"]}}])
      end
